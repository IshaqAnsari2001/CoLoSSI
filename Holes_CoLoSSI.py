...
if __name__ == "__main__":
    ...
    for i in All_Tasks:
        ...
        alias = str(get_alias(str(i),AGENT_OBJ[0].ID))
        if Data.expl[(AGENT_OBJ[0].ID,alias,str(i))] == 0:
    ...
    for ag in AGENT_OBJ:
        holecheck.append([int(x)%len(Data.CELLS) not in holes for x in ag.schedule] == [True] * len(ag.schedule))
    
    if holecheck == [True] * len(AGENT_OBJ):
        print("No hole violation")
    else:
        print("There is a hole violation")
    ...
    instance_name = str(str(args[0]).split("/")[-1].split(".")[0])
    #instance_name = "results/"+instance_name
    print("writing to files")
    makespan = get_max()
    FILE = open(instance_name+str(".makespan"), "w+")
    FILE.writelines(str(makespan))
    FILE.write("\n\n")
    FILE.close()
    FILE = open(instance_name+str(".schedule"), "w+")
    FILE.writelines(str(sch))
    FILE.write("\n\n")
    FILE.close()
    FILE = open(instance_name+str(".timings"), "w+")
    FILE.writelines(str(tim))
    FILE.write("\n\n")
    FILE.close()
    FILE = open(instance_name+str(".xdata"),"w+")
    FILE.writelines(str(args[0]))
    FILE.write("\n")
    FILE.close()
