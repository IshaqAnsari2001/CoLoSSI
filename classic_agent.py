...
def assign_tasks_to_1():
    ...
    for t in work:
        effic = round(1/Data.expl[(ag.ID,get_alias(str(t),ag.ID),str(t))])
        if t in ag.schedule:
            effic -= 1
        plan_time += [effic]
...
if __name__ == "__main__":
    ...
    #print(args)
    if len(args) not in [5,9]:
        print("Invalid number of arguments")
        sys.exit(0)
    Data.readMDATA(args[0])
    numb_agents = int(args[0][args[0].index("n")+1:args[0].index("t4")])
    
    if args[2] == "weight" and len(args) == 9:
        bidding = True
        Data.A = [x for x in args[7].split("-")]
        locs = [x for x in args[8].split("-")]
    elif args[2] == "noweight" and len(args) == 5:
        bidding = False
        Data.A = [x for x in args[3].split("-")]
        locs = [x for x in args[4].split("-")]
    else:
    ...
    if len(Data.A) > 1:
        ...
        print(str((ans == [True] * len(AGENT_OBJ)) and ([x < 0.0601 for x in TIMES2] == [True] * len(Data.CELLS))), end = " ")
        '''
        #print("LEFT ::: ",get_task_left())
        #print("NUM :: ", len(get_task_left()))
        #print(Tasks_Timings)
        Tasks_Timings = [16] * len(Data.CELLS)
        for ag in AGENT_OBJ:
            ag.timings = []
