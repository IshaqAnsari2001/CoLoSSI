    ...
    for ag in AGENTS:
        if int(ag.ID) in A:
            ...
            for cell_no in All_cells:
                ...
                if len(path) <= meetup_range[1] and len(path) >= meetup_range[0]:
                    l.append(cell_no)
                    alias = str(CoLoSSI.get_alias(str(cell_no),ag.ID))
                    effic  = round(1/Data.expl[(ag.ID,alias,str(cell_no))])
                    e.append((cell_no,effic))
            ag.valid_cells = l 
            ag.valid_tuple = e
            #print(ag.valid_tuple,len(ag.valid_tuple),"\n")

def filter_fun(input):
    if input[1] in effic_parameter: 
        return True
    else:
        return False

def filter_pairs(groups):
    A = [int(x) for x in groups.split("-")]
    for ag in AGENTS:
        if int(ag.ID) in A:
            ag.valid_tuple_filtered = list(filter(filter_fun,ag.valid_tuple))
            print(ag.valid_tuple_filtered)


def find_intersection(groups):
    A = [int(x) for x in groups.split("-")]
    d = []
    for ag in AGENTS:
        if int(ag.ID) in A:
            d.append(ag.valid_tuple_filtered)

    result = set(d[0]).intersection(*d[1:])
    
    #if list(result) = []:
    #    filter_pairs(groups,[4,8])
    for ag in AGENTS:
        if int(ag.ID) in A:
            ag.valid = list(result)
    print("INTER-SECTING POINTS ARE AS FOLLOWS ::::: ",result)



def generate_enclosure(input):
    center_loc = int(input[0])
...
def efficacy_normalizer_fucntion(cell_no):
    alias = str(CoLoSSI.get_alias(str(cell_no),ag.ID))
    effic  = round(1/Data.expl[(ag.ID,alias,str(cell_no))])
...
def Normalizer(enc, ag):
    res = []
    for cell_no in enc:
        alias = str(CoLoSSI.get_alias(str(cell_no),ag.ID))
        effic  = round(1/Data.expl[(ag.ID,alias,str(cell_no))])
        result = float((effic - 4)/(16 - 4))
        res.append(float(1.0 - result))
    return(res)
...
def normalize_efficacy(groups,g_Task_Timings):
    A = [int(x) for x in groups.split("-")]
    Efficacy = []
    f = True
    for ag in AGENTS:
        if int(ag.ID) in A:
            #print("Handeling agent number :: Agent",ag.ID)
            #print("CENTERS FOR THIS AGENT :: ",ag.valid)
            if len(ag.valid) == 1:
                print("ONLY ONE meetup point ::: ", ag.valid[0])
                f = False
                break
            temp = []
            for inputs in ag.valid:
                #print("Generating enclosure with center :: ",inputs," produces effic as :: ",end = " ")
                enc = generate_enclosure(inputs)
                completed = get_completed(g_Task_Timings)
                enc  = get_remaining_tasks(enc,completed)
                #if len(enc) == 0:
                #    print("all tasks with this center are completed")
                #print("mapping ")
                #effic = list(map(efficacy_normalizer_fucntion,enc))
                effic = Normalizer(enc,ag)
                effic = [float(x) for x in effic]
                res  = sum(effic)
                
                temp.append(res)
                print(res)
                print("\n")
            ag.meetup_effic = temp
            if len(ag.meetup_effic) !=len(ag.valid):
                print("Efficacy and valid len not same")
                print(ag.meetup_effic,len(ag.meetup_effic))
                print(ag.valid,len(ag.valid))
                exit(0)
            Efficacy.append(ag.meetup_effic)
    ...
    for ag in AGENTS:
        if int(ag.ID) in A:
            if len(ag.valid) == 1:
                ...
                flag = True
    if flag == True:
        array = np.array(Efficacy)
        res = list(np.sum(array,axis = 0))
        maximum_effic = max(res)
        idx = res.index(maximum_effic)
        for ag in AGENTS:
            if int(ag.ID) in A:
                ag.meetup_loc = ag.valid[idx][0]

    for ag in AGENTS:
        if int(ag.ID) in A:
            print("meetup loc for ag ",ag.ID," is :: ",ag.meetup_loc)

def BLACK_BOX(g,g_Task_Timings):
    get_valid(g)
    filter_pairs(g)
    find_intersection(g)
    normalize_efficacy(g,g_Task_Timings)
...
#    meeting = [random.randint(0,len(Data.CELLS)-1) for x in groups]    # Randomized meeting point
    ...
    for i in range(len(groups)):
        ...
        cmd = "python3 c_a.py " + args[0] + " " + groups[i] + " " + locs[i] + " " + str(interval) + " " + args[1] + " " + covs[i] + " " + str(meeting)
        #print(cmd)
        #pipe = Popen(cmd, shell=True, stdout=PIPE).stdout
        output = [x.replace("\n","") for x in os.popen(cmd).readlines()]
        curr_ag = [int(x)-1 for x in output[::3]]
    ...
    for ag in AGENTS:
        ...
        print("Tasks left",len(Data.CELLS) - len(get_completed(g_Task_Timings)))
